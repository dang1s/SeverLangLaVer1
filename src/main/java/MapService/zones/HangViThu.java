...
public class HangViThu extends Zone {
    private int mobid;
    private byte level = 70;

    public HangViThu(Map map, int zone) {
        super(map, zone);
        isHangViThu = true;
        timeReviveMob=5000;
    }
    ...
    public boolean addChar(Char player) {
        ...
        player.inHangViThu = true;
        // Set TypePK = 2 (PK mode) khi vào HangViThu
        player.InfoGame.TypePk = 2;
        player.service.sendMessage(HanderMessage.SendTypePk(player.Info.idEntity, (byte) 2));
        SendMessageInZone(HanderMessage.SendTypePk(player.Info.idEntity, (byte) 2));
...
//        }
    ...
    public void createMob(){
        ...
        if (map.mapID==90) {
            ...
            x = 60;
            for (int i = 0; i < 22; i++) {
                entityList.add(new XYEntity((short) (921 + x), (short) 677));
                x += 60;
            }
            x = 60;
            for (int i = 0; i < 6; i++) {
                entityList.add(new XYEntity((short) (303 + x), (short) 210));
                x += 60;
            }
            x = 60;
            for (int i = 0; i < 7; i++) {
                entityList.add(new XYEntity((short) (672 + x), (short) 210));
                x += 60;
            }
            x = 60;
            for (int i = 0; i < 7; i++) {
        ...
        for (int i = 0; i < entityList.size(); i++) {
            ...
            mob.status = 4;
            mob.hpGoc = mob.hp = mob.hpFull = level * mobid*850/3 *50;
            mob.expGoc = mob.hpGoc / 20;
...
//        // Map transition logic for HangViThu
//    }
    @Override
    ...
    public void getRewardMob(Char player, Mob mob) {
        ...
        int itemDrop = ItemDrop.ITEM_HANG_VI_THU.next();
        if (itemDrop == -1) {
            return;
        }
...
//                    item.isLock = true;
    ...
    public void mobAttackChar(Mob mob, Char player) {
        try {
            int dameAdjusted = mob.getDame()*20;// sua dame o day
