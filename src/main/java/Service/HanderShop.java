...
public class HanderShop {
    ...
    public static void BuyShop(Char _myChar, Message msg) {
        try {
                        ...
                        if (item.id == 163) {
                            ...
                            if (!item.getItemTemplate().isXepChong) {
                                for (int i = 0; i < item.amount; i++) {
                                    ...
                                    // Đảm bảo options được tạo lại cho clone
            ...
            if (itemShop.TypeShop == 40) {
                ...
                if (deductCurrency(_myChar, itemShop, it)) {
                    return;
                }
            ...
            if (itemShop.TypeShop != 41 && itemShop.TypeShop != 6) {
    ...
    private static boolean deductCurrency(Char _myChar, ItemShop itemShop, Item it) {
        try {
            if (itemShop != null) {
                if (itemShop.TypeShop == 41 && itemShop.TinhThach > 0) {
                    int totalNeeded = itemShop.TinhThach * it.amount;
                    if (_myChar.removeItems(884, totalNeeded)) {
                        _myChar.msgSendArrItemBag();
                    } else {
                        _myChar.user.session.sendMessage(HanderMessage.SendThongBao("Không đủ Thẻ điểm lục đạo", HanderMessage.RED_MID));
                        return true;
                    }
                } else if (itemShop.Bac > 0 && _myChar.Bag.bac >= itemShop.Bac * it.amount) {
                ...
                } else if (itemShop.TinhThach > 0 && _myChar.removeItems(160, itemShop.TinhThach * it.amount)) {
                    _myChar.msgSendArrItemBag();
                } else {
            ...
            _myChar.user.session.sendMessage(HanderMessage.SendThongBao("Lỗi hệ thống, vui lòng thử lại", HanderMessage.RED_MID));
            return true;
        }
    }

    private static void CheckItem(Item it, boolean hokage) {
        if (it.isVuKhi()) {
            it.strOptions = "";//xóa str op có sẵn
    ...
    private static void handleItemPurchase(Char _myChar, Item it, ItemShop itemShop) {
        ...
        if (!it.getItemTemplate().isXepChong) {
            ...
            // Đảm bảo options được tạo trước khi thêm vào túi
            it.createItemOptions();
    ...
    private static boolean CheckDiem(Char myChar, int type, int require) {
        switch (type) {
            case 8:
                return myChar.Info.pointHokage.Vk >= require;
            case 9:
                return myChar.Info.pointHokage.Dai >= require;
            case 10:
                return myChar.Info.pointHokage.Ao >= require;
            case 11:
                return myChar.Info.pointHokage.BaoTay >= require;
            case 12:
                return myChar.Info.pointHokage.Quan >= require;
            case 13:
                return myChar.Info.pointHokage.Giay >= require;
            case 14:
                return myChar.Info.pointHokage.Day >= require;
            case 15:
                return myChar.Info.pointHokage.Moc >= require;
            case 16:
                return myChar.Info.pointHokage.OngTieu >= require;
            case 17:
                return myChar.Info.pointHokage.Tui >= require;
            default:
                return false;
        }
    }
