...
public class DbMore {
    ...
    public static boolean InsertGiftCode(List<Code> codes) {
        try (Connection conn = Connect.getConnection();
             PreparedStatement ps = conn.prepareStatement("INSERT INTO giftcode set code = ? , infoitem = ?")) {
    ...
    public static void LoadGiftCode() {
        try (Connection conn = Connect.getConnection();
             ResultSet rs = conn.createStatement().executeQuery("Select * from giftcode")) {
            ObjectMapper json = new ObjectMapper();
    ...
    public static Code findCode(String codeStr) {
        for (Code c : DataCode.Codes) {
            if (c.Code.equals(codeStr)) {
                return c;
            }
        }

        try (Connection conn = Connect.getConnection();
             PreparedStatement ps = conn.prepareStatement("SELECT * FROM giftcode WHERE Code = ?")) {
    ...
    public static void saveGiftcode(Code code) {
        try (Connection conn = Connect.getConnection();
             PreparedStatement ps = conn.prepareStatement("UPDATE giftcode set InfoItem = ?where Code = ?")) {
            ObjectMapper json = new ObjectMapper();
            String info = json.writeValueAsString(code);
            ps.setString(1, info);
