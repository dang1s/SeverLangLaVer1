...
public class TetNguyenDan extends Event {
    public static final String TOP_LAM_KEM = "ice_cream";
    public static final String TOP_FISH = "topfish";

    public TetNguyenDan() {
        setId(Event.SU_KIEN_HE);
        endTime.set(2027, 30, 5, 23, 59, 59);
        itemsThrownFromMonsters.add(1, 830);
        itemsThrownFromMonsters.add(1, 831);
        itemsThrownFromMonsters.add(1, 832);
        itemsThrownFromMonsters.add(1, 833);
        itemsThrownFromMonsters.add(35, -1);//ko rơi
...
//        menuKhaTienNu = "Làm hũ kem,1 cái,10 cái,100 cái,Hướng dẫn;Giải cứu Tiên Nhân,Tham gia(500 vàng),Từ chối;BXH Top Làm Kem;BXH Top Câu Cá;Kiểm tra điểm sự kiện;Đổi điểm,Sách nhẫn thuật đặc biệt, Thẻ đổi tên, Cải trang Madara, Cải trang Madara Lục Đạo, Cải trang Kakashi Lục Đạo, Bí kíp Bí Ngô";
        menuKhaTienNu = " Làm Bánh Chưng,1 cái,10 cái,100 cái,Hướng dẫn;Giải cứu Anh Mộ Xum Xuê,Tham gia( Ngày 1 lần ),Từ chối;BXH Top Nấu Bánh;Khóa/mở khóa cấp;Kiểm tra điểm sự kiện;Đổi điểm,Sách nhẫn thuật đặc biệt, Thẻ đổi tên, Cải trang Madara, Cải trang Madara Lục Đạo, Cải trang Kakashi Lục Đạo, Bí kíp Bí Ngô;Đổi Sex 19,doi; doi tantovip,doi ";
    }

    private void makeIceCream(Char p, int amount) {
        int[][] itemRequires = new int[][]{{830, 5}, {831, 5}, {832, 5}, {833, 5}};
        int itemIdReceive = 834;
    ...
    private void makeIceCream2(Char p, int amount) {
        int[][] itemRequires = new int[][]{{435, 200}};
        int itemIdReceive = 939;

        boolean isDone = makeEventItem(p, amount, itemRequires, 50000, 0, 0, itemIdReceive);
        if (isDone) {
            p.getEventPoint().addPoint(TOP_LAM_KEM, amount);
            p.getEventPoint().addPoint(EventPoint.DIEM_TIEU_XAI, amount);
        }
        ...
        switch (item.id) {
            case 834:
                ...
                if (npc == null || Utlis.getRange(p.Info.cx, p.Info.cy, npc.cx, npc.cy) > 100) {
                    p.getService().serverMessage("Vui lòng đứng cạnh thầy Jiraiya để sử dụng Bánh Chưng");
                    return;
                }
    ...
    public void menu(Char p, int index, int index2) {
        switch (index) {
            case 0:
                switch (index2) {
                    ...
                    case 2:
                        ...
                        break;
                    case 3:
                        p.getService().sendTextNPC("Để làm 1 Bánh Chưng: 5 Lá + 5 Gạo + 5 Đậu + 5 Thịt  +  50 vàng", "");
                        break;
                }
                break;
            case 1:
                ...
                break;
            case 2:
                viewTop(p, TOP_LAM_KEM, "Bảng xếp hạng SK", "%d. %s có %s điểm làm Bánh");
                break;
            case 3:
                ...
                break;
            case 4:
                p.getService().sendTextNPC("Bảng điểm SK của bạn: ", "Điểm Làm Bánh: " + p.getEventPoint().getPoint(TOP_LAM_KEM) + "; Điểm câu cá: " + p.getEventPoint().getPoint(TOP_FISH)+";Điểm tiêu sài: "+p.getEventPoint().getPoint(EventPoint.DIEM_TIEU_XAI));
                break;
            case 5:
                int point = 0;
                switch (index2){
                    ...
                    case 2:
                        ...
                        madara.strOptions = "68,100;70,100;0,1000;2,200;4,200;5,200;209,100";
                        p.addItem(madara);
                    ...
                    case 3:
                        ...
                        madaralucdao.strOptions = "68,100;70,100;0,1000;2,200;4,200;5,200;209,100";
            ...
            case 6:
                ...
                break;
            case 7:
                Item veVanMayVip = p.FindItemBag(735);
                if (veVanMayVip == null) {
                    p.service.alertMessage("khong co kho ga");
                    return;
                }
                if (veVanMayVip.amount < 50000) {
                    p.service.alertMessage("Không đủ kho ga");
                    return;
                }
                p.removeItemByAmount(veVanMayVip, 50000);
                p.msgRemoveItemBag(veVanMayVip);
                Item kakashilucdao = new Item(284);
                kakashilucdao.strOptions = "204,133;258,466;6,6;0,3333;1,3333;209,166;81,6;119,394;120,393;121,454;122,374;123,40;124,39;125,39;126,40;127,39";
                p.addItem(kakashilucdao);
                p.msgAddItemBag(kakashilucdao);
                break;
