...
//                    clan.update();
    ...
    public List<SkillClan>skillClans;
    ...
    public void themItem(Item item) {
        ...
        if (index > -1) {
            this.items[index].add(item.getAmount());
        } else {
    ...
    public void addExp(int exp) {
        ...
        if(this.exp >= getExpNext()){
            ...
            this.level++;
            getClanService().serverMessage("Gia tộc đã nâng lên cấp "+this.level);
            writeLog("Thông báo","Gia tộc đã nâng lên cấp "+this.level+ " ngân sách bị trừ ",0);
        }
...
//                item.upgrade = 0;
//            }
//            item.setAmount(parse.getInt("quantity"));
//            if (item.hasExpire()) {
...
//        }
    ...
    public void writeLog(String name, String num, int number) {
        String[] array = log.split("\n");
        log = name + " : " + num + " " + number + " Bạc vào thời gian "
                + Utlis.dateToString(Date.from(Instant.now()), "yyyy/MM/dd hh:mm:ss") + "\n";
