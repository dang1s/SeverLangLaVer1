...
//                    .prepareStatement("SELECT * FROM `shinwa` WHERE `status` = ?");
...
//                Stall stall = findByType(item.getItemTemplate().type);
//                if (stall != null) {
//                    stall.add(item);
//                }
//            }
...
//        item.setProductID(autoIncrement());
//        item.setProductPrice(price);
//        item.setProductSeller(p.Info.name);
...
//        stalls.forEach((t) -> {
//            List<Item> expiredProductList = t.getExpiredProductListBySeller(p.Info.name);
