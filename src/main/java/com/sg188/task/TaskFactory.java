...
public class TaskFactory {
    ...
    public TaskOrder createTaskOrder(byte type, Char p) {
        ...
        if (type == TaskOrder.TASK_DAY) {
            ...
            task.setTask(0, Utlis.nextInt(20, 40), "Nhiệm vụ hàng ngày",
                    "Ghi chú: đi đến " + Map.maps[info.getMapID()].getMapTemplate().name + " để làm nhiệm vụ giết "+new Mob(info.getMobID()).getMobTemplate().name,
                    info.getMobID(), info.getMapID());
        } else if (type == TaskOrder.TASK_BOSS) {
            MobInfo info = getMobInfoTaskBossClosestLevel(p.level());
            task.setTask(0, 1, "Nhiệm vụ truy bắt linh thú",
                    "Ghi chú: đi đến " + Map.maps[info.getMapID()].getMapTemplate().name + " để làm nhiệm vụ giết "+new Mob(info.getMobID()).getMobTemplate().name,
                    info.getMobID(), info.getMapID());
        }
        return task;
